...
class BaostockFetcher(StockFetcher):
    ...
    def fetch_stock_ts_code_list(self) -> List[str]:
        ...
        self.logger.info(f"fetch_stock_ts_code_list {result.shape}")
        return result

    
    def fetch_by_ts_code_and_freq(self, ts_code: str, start_date: str, end_date: str, freq:str) -> pd.DataFrame:
        time.sleep(random.randint(1, 3))
        self.logger.info(f"fetch_by_ts_code_and_freq {ts_code} {start_date} {end_date} {freq}")
        ...
        while (rs.error_code == '0') & rs.next():
            ...
            data_list.append(rs.get_row_data())
        result = pd.DataFrame(data_list, columns=rs.fields)
        self.logger.info(f"fetch_by_ts_code_and_freq {ts_code} {start_date} {end_date} {freq} {result.shape}")
        return result
